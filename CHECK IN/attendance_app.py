...
(function(){{
  ...
  const nodes = data.map(d => ({{
    ...d, r: rScale(d.count),
    x: (HAS_ZONES ? zoneX[d.zone] : W/2) + (Math.random()-0.5)*40,
    y: cy + (Math.random()-0.5)*H*0.3
  }}));

  const sim = d3.forceSimulation(nodes)
    .force('x', d3.forceX(d => HAS_ZONES ? zoneX[d.zone] : W/2).strength(HAS_ZONES ? 0.18 : 0.06))
    .force('y', d3.forceY(cy).strength(0.04))
    .force('charge', d3.forceManyBody().strength(-4))
    .force('collide', d3.forceCollide(d => d.r + 3).strength(0.95).iterations(6))
    .stop();
  for (let i = 0; i < 400; ++i) sim.tick();

  nodes.forEach(d => {{
    d.x = Math.max(d.r + PAD, Math.min(W - d.r - PAD, d.x));
    d.y = Math.max(d.r + 28, Math.min(H - d.r - 6, d.y));
  }});
...
def render_checkin_time_chart(tab_name, page_colors, target_date=None):
    ...
    chart_end = (max(arrival_minutes) // BUCKET) * BUCKET
    buckets = list(range(chart_start, chart_end + 1, BUCKET))
    if not buckets:
        return
    ...
    for mins in arrival_minutes:
        b = chart_start + ((mins - chart_start) // BUCKET) * BUCKET
...
def render_dashboard(tab_name, group_by_zone=False):
    ...
    if total_checked_in > 0:
        ...
        with st.expander(names_title, expanded=False):
        ...
        if all_members_by_cell_group:
            ...
            with st.expander(names_title_empty, expanded=False):
