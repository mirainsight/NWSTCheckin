...
_CR_VALUE_KEYWORDS: dict[str, dict[str, str]] = {
    "Status": {
        ...
        "moved to ya": "Graduated: Will no longer follow up. Moved to different ministry",
        "campus community": "Graduated: Will no longer follow up. Moved to different ministry",
        "young adults": "Graduated: Will no longer follow up. Moved to different ministry",
        "campus": "Graduated: Will no longer follow up. Moved to different ministry",
...
def _cr_expand_to_option(field: str, partial: str) -> str:
    ...
    for o in options:
        ...
        if head == pl:
            return o
    # whole-word match anywhere in option (e.g. "Worship: Vocals" matched by "vocals" but not "vocalising")
    for o in options:
        if re.search(r'\b' + re.escape(pl) + r'\b', o.lower()):
            return o
...
def _render_cr_wizard() -> None:
    ...
    # Step 4 — New value
        ...
        _st_components.html(
            ...
            f'-webkit-mask-image:linear-gradient(to right,transparent 0,black 6%,black 94%,transparent 100%);}}'
            f'.track{{display:flex;gap:12px;width:max-content;'
            f'animation:marquee 10s linear infinite;}}'
            f'.wrap:hover .track{{animation-play-state:paused;}}'
            f'@keyframes marquee{{0%{{transform:translateX(0)}}100%{{transform:translateX(-50%)}}}}'
            f'.chip{{flex:0 0 auto;background:#1a1a1a;'
